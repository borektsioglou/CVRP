...
public class CVRPData {
    ...
    public static int getDemand(int node) {
		if (!nodeIsValid(node)) {
		    System.err.println("Error: demand for node " + node + 
				       " was requested from getDemand() but only nodes 0.." + (NUM_NODES - 1) + " exist");
		    System.exit(-1);
		}	    
		return demand[node];
    }
    ...
    static double[][] getEuclideanDistanceMatrix() {
        double[][] matrix = new double[VEHICLE_CAPACITY][VEHICLE_CAPACITY];
    ...
    public static double getDistance(int node1, int node2) {
		if (!nodeIsValid(node1)) {
		    System.err.println("Error: distance for node " + node1 + 
				       " was requested from getDistance() but only nodes 0.." + (NUM_NODES - 1) + " exist");
		    System.exit(-1);
		}	    
    ...
    public static int[] flattenPath(int[][] paths){
    	int k = 0;
    	int [] newPath = new int[NUM_NODES - 1];
	...
	public static boolean pathIsValid(ArrayList<Integer> path, int j) {
		
		return getPathDemand(path) + demand[j-1] <= VEHICLE_CAPACITY;
	}

	private static int getPathDemand(ArrayList<Integer> path) {
		int totDemand = 0;
		for(int i = 0; i < path.size(); i++){
			totDemand += demand[path.get(i)-1];
		}
		return totDemand;
	}
